...
`Adapter` 是组件挂载适配器。Makoo 通过 `@makoo/vue` 和 `@makoo/react` 支持 Vue / React，后续也可以扩展其他可挂载产物。
...
| 包 | 职责 |
| --- | --- |
| `@makoo/core` | 框架无关的注入运行时 |
| `@makoo/vue` | Vue 挂载适配器 |
| `@makoo/react` | React 挂载适配器 |
| `@makoo/cli` | Vite 插件、配置解析、扫描、代码生成 |
| `@makoo/create-makoo` | 项目脚手架 |

大多数 userscript 项目优先使用 `@makoo/cli`。只有自定义运行时集成时，才需要直接接触 `@makoo/core`、`@makoo/vue` 或 `@makoo/react`。
