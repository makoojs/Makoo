	...
	<div class="makoo-home-shell overflow-hidden" data-od-id="makoo-home">
		<section class="hero mx-auto grid w-[min(calc(100%-40px),var(--max))] grid-cols-[minmax(0,.9fr)_minmax(440px,1.1fr)] items-center gap-18 pt-12 pb-19.5" data-od-id="home-hero">
			...
			<div class="runtime-visual relative min-h-112.5 overflow-hidden rounded-4.5" :aria-label="content.runtimeDescription">
				...
				<div class="runtime-stage"><div class="browser-panel"><p class="panel-label">宿主页面 DOM</p><div class="dom-tree" aria-hidden="true"><div class="dom-line">&lt;body&gt;</div><div class="dom-line">&lt;main&gt;</div><div class="dom-line target">#target detected</div><div class="dom-line">&lt;/main&gt;</div></div></div><div class="pipeline" aria-hidden="true"><span class="pipeline-label">adapter.mount()</span></div><div class="mount-panel"><p class="panel-label">组件注入结果</p><div class="component-shell" aria-hidden="true"><div class="component-card"><div class="component-head"></div><div class="component-copy"></div><div class="component-copy short"></div><div class="component-action"></div></div></div></div></div>
				<div class="event-log" aria-hidden="true"><div class="log-item"><span class="log-mark"></span><span>等待目标节点</span></div><div class="log-item"><span class="log-mark"></span><span>匹配组件 adapter</span></div><div class="log-item"><span class="log-mark"></span><span>挂载并保持 alive</span></div></div>
			</div>
		</section>
		...
		<section id="developers" class="section relative isolate w-full overflow-hidden pb-28" data-od-id="developers-section">
			...
			<div class="capability-grid">
				<article class="capability-card task-card reveal"><div class="capability-copy"><h3>{{ content.capabilities[0][0] }}</h3><p>{{ content.capabilities[0][1] }}</p></div><div class="capability-visual code-window"><div class="code-window-bar"><span>headerBadge.ts</span><span>TypeScript</span></div><div class="code-lines"><div class="code-line" data-line="1"><span class="code-key">inject</span>({</div><div class="code-line active" data-line="2">&nbsp;&nbsp;id: <span class="code-value">'header-badge'</span>,</div><div class="code-line active" data-line="3">&nbsp;&nbsp;injectAt: <span class="code-value">'#header'</span>,</div><div class="code-line" data-line="4">&nbsp;&nbsp;component: HeaderBadge,</div><div class="code-line" data-line="5">&nbsp;&nbsp;alive: <span class="code-key">true</span></div><div class="code-line" data-line="6">});</div></div></div></article>
				...
				<article class="capability-card adapter-card reveal"><div class="capability-copy"><h3>{{ content.capabilities[2][0] }}</h3><p>{{ content.capabilities[2][1] }}</p></div><div class="capability-visual adapter-lab" role="img" aria-label="adapter 通用协议"><span class="adapter-beam matches"></span><span class="adapter-beam mount"></span><span class="adapter-beam unmount"></span><div class="adapter-hub">adapter<br />协议</div><span class="adapter-method matches">matches()</span><span class="adapter-method mount">mount()</span><span class="adapter-method unmount">unmount()</span></div></article>
				<article class="capability-card lifecycle-card reveal"><div class="capability-copy"><h3>{{ content.capabilities[3][0] }}</h3><p>{{ content.capabilities[3][1] }}</p></div><div class="capability-visual state-demo"><div class="state-controls" role="group" :aria-label="content.stateLabel"><button v-for="state in lifecycleSequence" :key="state" class="state-chip" type="button" :aria-pressed="lifecycleState === state" @click="selectLifecycleState(state)"><span class="state-chip-dot"></span>{{ content.states[state].button }}</button></div><div class="state-panel" :data-state="lifecycleState"><div class="state-panel-head"><div class="state-panel-title"><span>{{ content.statePanelLabel }}</span><strong>{{ activeLifecycle.title }}</strong></div><span class="state-status">{{ activeLifecycle.status }}</span></div><div class="runtime-topology" aria-hidden="true"><div class="life-node host-node"><span class="node-kind">页面位置</span><strong>页头区域</strong><span class="state-value">{{ activeLifecycle.host }}</span></div><div class="observer-bridge"><span class="observer-pulse"></span><span class="state-value">{{ activeLifecycle.observer }}</span></div><div class="life-node instance-node"><span class="node-kind">界面组件</span><strong>顶部徽标</strong><span class="state-value">{{ activeLifecycle.instance }}</span></div></div><div class="state-trace"><div class="trace-head"><span>当前过程</span><span>{{ activeLifecycle.trace }}</span></div><ol class="trace-list"><li v-for="(event, index) in activeLifecycle.events" :key="event[0]" class="trace-item"><span class="trace-index">{{ String(index + 1).padStart(2, '0') }}</span><span class="trace-copy"><strong>{{ event[0] }}</strong><small>{{ event[1] }}</small></span></li></ol></div><p class="state-explanation" aria-live="polite">{{ activeLifecycle.copy }}</p></div></div></article>
			</div>
		</section>
