...
const locale = computed<Locale>(() => {
	if (localeIndex.value === 'zh' || lang.value?.toLowerCase().startsWith('zh')) return 'zh';
	const p = route.path;
	if (p.startsWith('/zh/') || p === '/zh' || p.includes('/zh/')) return 'zh';
	return 'en';
});
...
const messages = {
	en: {
		eyebrow: 'Component-driven userscript framework', title: 'Add your own touches\nto everyday pages.',
		lead: 'Write components in Vue or React.\nLet Makoo handle injection and lifecycle.',
		getStarted: 'Get started', github: 'GitHub', copy: 'Copy', copying: 'Copying', copied: 'Copied', copyError: 'Copy failed',
	...
	zh: {
		eyebrow: 'Userscript 开发框架', title: '用组件扩展网页',
		lead: '使用 Vue 或 React 构建界面\n由 Makoo 管理注入与生命周期',
		getStarted: '开始使用', github: 'GitHub', copy: '复制', copying: '正在复制', copied: '已复制', copyError: '复制失败',
	...
	<div class="makoo-home-shell" :lang="locale">
		<section class="hero" aria-labelledby="home-title">
			...
			<div class="hero-art" aria-hidden="true">
				<img
					:src="withBase('/makoo-hero-relief-1254.webp')"
					:srcset="`${withBase('/makoo-hero-relief-640.webp')} 640w, ${withBase('/makoo-hero-relief-1254.webp')} 1254w`"
					sizes="(max-width: 767px) 100vw, (max-width: 959px) 70vw, 65vw"
					width="1254"
					height="1254"
					alt=""
					fetchpriority="high"
